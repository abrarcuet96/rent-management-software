...
async def clean_owner_data(conn: asyncpg.Connection, owner_id: str) -> None:
    ...
    tables = [
        ("due_expense", "due_id IN (SELECT id FROM monthly_due WHERE tenant_id IN (SELECT id FROM tenant WHERE apartment_id IN (SELECT id FROM apartment WHERE building_id IN (SELECT id FROM building WHERE owner_id = $1))))"),
        ("payment_record", "due_id IN (SELECT id FROM monthly_due WHERE tenant_id IN (SELECT id FROM tenant WHERE apartment_id IN (SELECT id FROM apartment WHERE building_id IN (SELECT id FROM building WHERE owner_id = $1))))"),
        ("monthly_due", "tenant_id IN (SELECT id FROM tenant WHERE apartment_id IN (SELECT id FROM apartment WHERE building_id IN (SELECT id FROM building WHERE owner_id = $1)))"),
        ("tenant_agreement", "tenant_id IN (SELECT id FROM tenant WHERE apartment_id IN (SELECT id FROM apartment WHERE building_id IN (SELECT id FROM building WHERE owner_id = $1)))"),
        ("tenant", "apartment_id IN (SELECT id FROM apartment WHERE building_id IN (SELECT id FROM building WHERE owner_id = $1))"),
        ("expense", "owner_id = $1"),
...
async def seed_all(conn: asyncpg.Connection, owner_id: str) -> dict[str, Any]:
    ...
    await conn.execute(
        ...
        data["apt_101"], data["apt_102"], data["apt_201"], data["apt_301"],
        data["apt_A1"], data["apt_A2"], data["b1"], data["b2"],
    )
    ...
    await conn.execute(
        ...
        data["t_karim"], data["t_nasrin"], data["t_sabbir"], data["t_jalal"],
        data["apt_101"], data["apt_201"], data["apt_301"], data["apt_A1"],
    )
    ...
    await conn.execute(
        ...
        data["agr_karim1"], data["agr_karim2"], data["agr_nasrin"], data["agr_sabbir"],
        data["agr_jalal"], data["t_karim"], data["t_nasrin"], data["t_sabbir"], data["t_jalal"],
    )
...
async def seed_dues(conn: asyncpg.Connection, d: dict[str, Any]) -> None:
    ...
    # Jan: paid
    add_due("t_karim", "agr_karim1", 1, 2025, 15000, 15000, 15000, 0, "paid", "2025-01-01",
            [(15000, "2025-01-10", None)])
    # Feb: partial (paid 10000)
    add_due("t_karim", "agr_karim1", 2, 2025, 15000, 15000, 10000, 5000, "partial", "2025-02-01",
            [(10000, "2025-02-15", None)])
    # Mar: unpaid (overdue)
    add_due("t_karim", "agr_karim1", 3, 2025, 15000, 15000, 0, 15000, "unpaid", "2025-03-01")
    # Apr: paid
    add_due("t_karim", "agr_karim1", 4, 2025, 15000, 15000, 15000, 0, "paid", "2025-04-01",
            [(15000, "2025-04-10", None)])
    # May: paid
    add_due("t_karim", "agr_karim1", 5, 2025, 15000, 15000, 15000, 0, "paid", "2025-05-01",
            [(15000, "2025-05-10", None)])
    # Jun: paid
    add_due("t_karim", "agr_karim1", 6, 2025, 15000, 15000, 15000, 0, "paid", "2025-06-01",
            [(15000, "2025-06-10", None)])
    # Jul: paid (new rent 18000)
    add_due("t_karim", "agr_karim2", 7, 2025, 18000, 18000, 18000, 0, "paid", "2025-07-01",
            [(18000, "2025-07-10", None)])
    # Aug: partial (2 payments: 8000 + 4000)
    add_due("t_karim", "agr_karim2", 8, 2025, 18000, 18000, 12000, 6000, "partial", "2025-08-01",
    ...
    # Jan: paid
    add_due("t_karim", "agr_karim2", 1, 2026, 18000, 18000, 18000, 0, "paid", "2026-01-01",
            [(18000, "2026-01-10", None)])
    # Feb: partial (paid 10000)
    add_due("t_karim", "agr_karim2", 2, 2026, 18000, 18000, 10000, 8000, "partial", "2026-02-01",
    ...
    for m in range(1, 13):
        add_due("t_nasrin", "agr_nasrin", m, 2025, 12000, 12000, 12000, 0, "paid", f"2025-{m:02d}-01",
                [(12000, f"2025-{m:02d}-10", None)])

    # ── Nasrin: 2026 (rent 12000, all paid) ──
    for m in range(1, 6):
        add_due("t_nasrin", "agr_nasrin", m, 2026, 12000, 12000, 12000, 0, "paid", f"2026-{m:02d}-01",
                [(12000, f"2026-{m:02d}-10", None)])

    # ── Sabbir: 2025 (moved in Mar, rent 10000) ──
    # Mar: paid
    add_due("t_sabbir", "agr_sabbir", 3, 2025, 10000, 10000, 10000, 0, "paid", "2025-03-01",
            [(10000, "2025-03-10", None)])
    # Apr: paid
    add_due("t_sabbir", "agr_sabbir", 4, 2025, 10000, 10000, 10000, 0, "paid", "2025-04-01",
            [(10000, "2025-04-10", None)])
    # May: paid
    add_due("t_sabbir", "agr_sabbir", 5, 2025, 10000, 10000, 10000, 0, "paid", "2025-05-01",
            [(10000, "2025-05-10", None)])
    # Jun: partial (paid 6000)
    add_due("t_sabbir", "agr_sabbir", 6, 2025, 10000, 10000, 6000, 4000, "partial", "2025-06-01",
            [(6000, "2025-06-15", None)])
    # Jul-Dec: unpaid (overdue)
    for m in range(7, 13):
        add_due("t_sabbir", "agr_sabbir", m, 2025, 10000, 10000, 0, 10000, "unpaid", f"2025-{m:02d}-01")

    # ── Sabbir: 2026 (rent 10000) ──
    # Jan: paid
    add_due("t_sabbir", "agr_sabbir", 1, 2026, 10000, 10000, 10000, 0, "paid", "2026-01-01",
            [(10000, "2026-01-12", None)])
    # Feb-May: unpaid (overdue / current)
    for m in range(2, 6):
        add_due("t_sabbir", "agr_sabbir", m, 2026, 10000, 10000, 0, 10000, "unpaid", f"2026-{m:02d}-01")

    # ── Jalal: 2025 only (moved out Aug, rent 8000, all paid) ──
    for m in range(1, 9):
        add_due("t_jalal", "agr_jalal", m, 2025, 8000, 8000, 8000, 0, "paid", f"2025-{m:02d}-01",
...
async def seed_expense_categories(conn: asyncpg.Connection, owner_id: str, d: dict[str, Any]) -> None:
    cats = [
        ("Electricity", True),
        ("Water", True),
        ("Maintenance", False),
    ...
    for name, is_default in cats:
        ...
        await conn.execute(
            ...
            cat_id, owner_id, name, is_default,
        )
...
async def seed_expenses(conn: asyncpg.Connection, owner_id: str, d: dict[str, Any]) -> None:
    expenses = [
        ...
        ("exp_elec1", d["b1"], None, "electricity", "বিল্ডিং বিদ্যুৎ বিল", 5000, "2025-03-15", "building", False),
        ("exp_elec2", d["b1"], None, "electricity", "বিল্ডিং বিদ্যুৎ বিল", 5500, "2026-02-15", "building", False),
        ("exp_water1", d["b1"], None, "water", "পানির বিল", 3000, "2025-04-20", "building", False),
        ("exp_repair1", d["b1"], d["apt_101"], "repair", "এপার্টমেন্ট মেরামত", 2000, "2025-02-10", "apartment", True),
        ("exp_maint1", d["b2"], None, "maintenance", "রক্ষণাবেক্ষণ খরচ", 4000, "2025-05-10", "building", False),
        ("exp_gas1", d["b1"], d["apt_201"], "gas", "গ্যাস সংযোগ মেরামত", 1500, "2025-06-15", "apartment", True),
        ("exp_maint2", d["b1"], None, "maintenance", "লিফট মেরামত", 8000, "2026-01-20", "building", False),
        ("exp_gas2", d["b1"], d["apt_301"], "gas", "গ্যাস বিল", 1200, "2026-03-10", "apartment", True),
    ]
    d["exp_ids"] = {}
...
async def seed_bulk_demo(conn: asyncpg.Connection, d: dict[str, Any]) -> None:
    ...
    sabbir_due_ids = [
        d["due_ids"].get(f"t_sabbir_2025_{m}") for m in [4, 5, 6]
    ]
    await conn.execute(
    ...
    karim_due_ids = [
        d["due_ids"].get(f"t_karim_2025_{m}") for m in [7, 8]
    ]
    await conn.execute(
    ...
    nasrin_due_ids = [
        d["due_ids"].get(f"t_nasrin_2026_{m}") for m in [1, 2, 3]
    ]
    await conn.execute(
...
async def main(email: str, password: str) -> None:
    ...
    try:
        ...
        paid = await conn.fetchval(
            "SELECT COUNT(*) FROM monthly_due WHERE tenant_id IN (SELECT id FROM tenant WHERE apartment_id IN (SELECT id FROM apartment WHERE building_id IN (SELECT id FROM building WHERE owner_id = $1))) AND status = 'paid'", owner_id
        )
        partial = await conn.fetchval(
            "SELECT COUNT(*) FROM monthly_due WHERE tenant_id IN (SELECT id FROM tenant WHERE apartment_id IN (SELECT id FROM apartment WHERE building_id IN (SELECT id FROM building WHERE owner_id = $1))) AND status = 'partial'", owner_id
        )
        unpaid = await conn.fetchval(
            "SELECT COUNT(*) FROM monthly_due WHERE tenant_id IN (SELECT id FROM tenant WHERE apartment_id IN (SELECT id FROM apartment WHERE building_id IN (SELECT id FROM building WHERE owner_id = $1))) AND status = 'unpaid'", owner_id
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--password", default=DEFAULT_PASSWORD, help="Owner password (only for new accounts)")
